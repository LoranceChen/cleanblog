...
class ApplicationServer(val port: Int, resourceBase: String) {
  ...
  private val atmosphereHolder2 = {
    import io.udash.rpc._
    import com.example.rpc._
    import scala.concurrent.ExecutionContext.Implicits.global

    val config = new DefaultAtmosphereServiceConfig[PingPongServerRPC]((clientId) => new DefaultExposesServerRPC[PingPongServerRPC](new PingPongEndpoint()))
    val framework = new DefaultAtmosphereFramework(config)
    //Disabling all files scan during service auto-configuration,
    //as it's quite time-consuming - a few seconds long.
    //
  ...
  contextHandler.addServlet(atmosphereHolder2, "/atm2/*")
  //aaa
}
